...
export default function OptionScreen() {
	...
	const { isLoading, consentToRGPDGiven, restoreConsentFromFirebase } =
	...
	useEffect(() => {
		// Restore consent state from Firebase and AsyncStorage on component mount
		restoreConsentFromFirebase();
	}, [restoreConsentFromFirebase]);
	return (
		<>
			<Stack.Screen
							...
							>
								<View className="flex-row items-center mb-2">
									...
									<Text className="m-2 text-white">
										Version: {Application.applicationName} v
					...
					<CardContent>
						{supportLinks.map((link, index) => (
							<Pressable
								key={index}
								onPress={() =>
									link.external ? Linking.openURL(link.url!) : link.onPress?.()
								}
