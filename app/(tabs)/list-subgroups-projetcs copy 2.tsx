...
const transformGroupData = (response: GroupResponse) => {
    ...
    // Transform subgroups
    response.subgroups?.forEach(group => {
        items.push({
    ...
    try {
        ...
        const items: GroupItem[] = [
            ...projects.map(project => ({
                ...
                visibility: project.visibility_level as Visibility,
                updatedAt: project.last_activity_at,
                stars: project.star_count,
                avatar: project.avatar_url,
                description: project.description,
                membershipType: directMemberIds.has(project.id) ? 'direct' : 'inherited'
            })),
            ...subgroups.map(group => ({
                id: group.id.toString(),
                name: group.name,
...
const fetchProjects = async (groupId: string) => {
    const { data } = await gitlabApi.get<Project[]>(`/groups/${groupId}/projects`, {
        params: {
...
const fetchSubgroups = async (groupId: string) => {
    const { data } = await gitlabApi.get<SubGroup[]>(`/groups/${groupId}/subgroups`, {
        params: {
            all_available: true,
            min_access_level: 10
        }
...
const ProjectIcon = ({ name }: { name: string }) => {
    // Default icon background color based on name
    ...
    return (
        <View style={[styles.iconContainer, { backgroundColor: getColor(name) }]}>
            ...
            >
                <View style={styles.leftContent}>
                    ...
                    <View style={styles.nameContainer}>
                        ...
                        {item.type === 'project' && item.isOwner && (
...
export default function GroupScreen() {
    return (
        // <GroupDetailView groupId="expo-app1" />
