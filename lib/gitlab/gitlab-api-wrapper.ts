...
let defaultPosthog = null;
function getDefaultPosthog() {
	if (!defaultPosthog && process.env.EXPO_PUBLIC_POSTHOG_API_KEY) {
		try {
			defaultPosthog = new PostHog(process.env.EXPO_PUBLIC_POSTHOG_API_KEY, {
	...
	return defaultPosthog;
}
class GitLabClient {
	constructor(options = {}, logger = getDefaultPosthog()) {
	...
	Discussions = {
		show: async (projectId, issueIid, discussionId) => {
			return this.request(
				`/projects/${projectId}/issues/${issueIid}/discussions/1`,
			);
		},
		create: async (projectId, issueIid, description) => {
			return this.request(
				`/projects/${projectId}/issues/${issueIid}/discussions?body=${description}`,
				"POST",
			);
		},
		add: async (projectId, issueIid, discussionId, query) => {
			const queryString = new URLSearchParams(options).toString();
			return this.request(
				`/projects/${projectId}/issues/${issueIid}/discussions/${discussionId}?${queryString}`,
				"POST",
			);
		},
	};
	...
	ProjectMergeRequests = {
		all: async (projectId, options = {}) => {
			const queryString = new URLSearchParams(options).toString();
			return this.request(
				`/projects/${projectId}/merge_requests?${queryString}`,
			);
		},
		create: async (projectId, data) => {
			return this.request(
	...
	Branches = {
		all: async (projectId, options = {}) => {
			const queryString = new URLSearchParams(options).toString();
			return this.request(
				`/projects/${projectId}/repository/branches?${queryString}`,
			);
		},
	...
	Notifications = {
		all: async (projectId, options = {}) => {
			const queryString = new URLSearchParams(options).toString();
			return this.request(
				`/projects/${projectId}/notification_settings?${queryString}`,
			);
		},
		show: async (projectId, userId) => {
			return this.request(
				`/projects/${projectId}/notification_settings/${userId}`,
			);
		},
	...
	GroupNotifications = {
		all: async (groupId, options = {}) => {
			const queryString = new URLSearchParams(options).toString();
			return this.request(
				`/groups/${groupId}/notification_settings?${queryString}`,
			);
		},
		update: async (groupId, data) => {
			return this.request(
				`/groups/${groupId}/notification_settings`,
				"PUT",
				data,
			);
		},
	...
	Users = {
		...
		contributed_projects: async (userId, options = {}) => {
			return this.request(
				`/users/${userId}/contributed_projects`,
				"GET",
				options,
			);
		},
	...
	IssueLinks = {
		all: async (projectId, issueIid, options = {}) => {
			const queryString = new URLSearchParams(options).toString();
			return this.request(
				`/projects/${projectId}/issues/${issueIid}/links?${queryString}`,
			);
		},
		show: async (projectId, issueIid, linkId) => {
			return this.request(
				`/projects/${projectId}/issues/${issueIid}/links/${linkId}`,
			);
		},
	...
	RepositoryFile = {
		...
		blame: async (projectId, filePath, options = {}) => {
			return this.request(
				`/projects/${projectId}/repository/files/${encodeURIComponent(filePath)}/blame`,
				"GET",
				options,
			);
		},
	...
	createProjectMergeRequest = async (
		projectId,
		source_branch,
		target_branch,
		title,
